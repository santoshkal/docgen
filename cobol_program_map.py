...
def generate_cobol_program_map(
    ...
    Args:
        ...
        top_n_paragraphs: Number of top paragraphs to show
        top_n_data_items: Number of top data items to show
    ...
    if 'DATA DIVISION' in divisions or ranked_data:
        ...
        map_lines.append("│   │")
        map_lines.append(f"│   ├── Top {min(top_n_data_items, len(ranked_data))} Most-Used Data Items:")

        # Get full symbol info for top ranked items
        data_symbols = {s['name']: s for s in ctags_outline.get('symbols', [])
                       if s.get('kind') == 'data'}

        for rank, (data_name, score) in enumerate(ranked_data[:top_n_data_items], 1):
    ...
    map_lines.append("    │")
    map_lines.append(f"    ├── Top {min(top_n_paragraphs, len(ranked_paragraphs))} Most Important Paragraphs:")

    # Get full symbol info for paragraphs
    para_symbols = {s['name']: s for s in ctags_outline.get('symbols', [])
                   if s.get('kind') in ['paragraph', 'section']}

    for rank, (para_name, score) in enumerate(ranked_paragraphs[:top_n_paragraphs], 1):
        line = None
